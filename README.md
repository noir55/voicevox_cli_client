...
VOICEVOX ENGINEを利用して、テキストを音声ファイルにしたり再生することができます。
...
| -1 | 文章全体を1つのファイルに出力します。指定しない場合は、空行を区切りとして連番のファイルに出力します |
...
単語を一括登録リストからまとめて登録 (一括登録リストは `単語` `読み方(カタカナ)` `アクセント核位置` をタブ区切りで1行に1単語づつ記述)
```
