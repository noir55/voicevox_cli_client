...
VOICEVOX ENGINE、COEIROINK ENGINEは別途用意して起動してください。(VOICEVOX バージョン 0.13.2、COEIROINK バージョン 1.6.0 で動作チェックしています)

COEIROINKサーバはLinux版がありません。Windows版の場合、コマンドプロンプトで以下のように実行することで起動できます。IPアドレスは、Windowsマシンに割り当てたものを指定してください。
...
VOICEVOX ENGINE を使用しない場合は `VOICEVOX_ENGINE = [ ]` と、値を空にしてください。

COEIROINK を使用する場合は同様に、

```
COEIROINK_ENGINE = [ "192.168.0.21:50031" ]
```

のように設定してください。カンマ区切りで複数指定も可能です。COEIROINK を使用しない場合は `COEIROINK_ENGINE = [ ]` と、値を空にしてください。
...
コマンドで、設定した VOICEVOX ENGINE、COEIROINK ENGINEが動作しているかどうか確認できます。VOICEVOX ENGINEを複数使用する場合はバージョンを揃えてください。COEIROINK ENGINEを複数使用する場合は、バージョン
・インストール済み音声ライブラリを揃えてください。
